...
class Major extends CI_Controller {
    ...
    private static $title = "Program Studi &minus; Pengumpulan Berkas Skripsi Unisbank Semarang";
    ...
    public function add()
    {
        ...
        if ($this->validation()) {
            ...
            if (strtolower($data['m_name']) === strtolower($this->input->post('m_name', TRUE))) {
                $this->session->set_flashdata('alert', error('Nama Progdi sudah ada!'));
                $data['title'] = "Tambah ".self::$title;
                $data['form_title'] = "Tambah Progdi";
                $data['action'] = site_url(uri_string());
                $data['content'] = 'dashboard/major-form';
                $this->load->view('dashboard/index', $data);

            } else if (strtolower($data['m_id']) === strtolower($this->input->post('m_id', TRUE))) {
                $this->session->set_flashdata('alert', error('Inisial Progdi sudah ada!'));
                $data['title'] = "Tambah ".self::$title;
                $data['form_title'] = "Tambah progdi";
                $data['action'] = site_url(uri_string());
                $data['content'] = 'dashboard/major-form';
                ...
                $this->session->set_flashdata('alert', success('Data progdi berhasil ditambahkan.'));
                $data['title'] = "Data ".self::$title;
                $data['content'] = "dashboard/major";
                redirect('major');
            }
    ...
    public function edit()
    {
        ...
        if ($this->validation()) {
            ...
            $this->session->set_flashdata('alert', success('Data progdi berhasil diperbarui.'));
            $data['title'] = "Data ".self::$title;
            $data['content'] = "dashboard/major";
            redirect('major');
    ...
    public function delete($m_id)
    {
        ...
        $this->session->set_flashdata('alert', success('Data progdi berhasil dihapus.'));
        $data['title'] = "Data ".self::$title;
        $data['content'] = "dashboard/major";
        $this->load->view('dashboard/index', $data);
        redirect('major');
    ...
    public function restore()
    {
        ...
        $this->session->set_flashdata('alert', success('Data progdi berhasil direstore.'));
        $data['title'] = "Data ".self::$title;
        $data['content'] = "dashboard/major-deleted";
    ...
    private function validation()
    {
        ...
        $this->form_validation->set_rules('m_name', 'Nama progdi', 'trim|required|min_length[5]|max_length[50]');
        $this->form_validation->set_rules('m_id', 'Inisial progdi', 'trim|required|alpha|max_length[5]');
