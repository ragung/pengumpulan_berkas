...
class Student extends CI_Controller {
    ...
    private static $title = "Mhs &minus; Pengumpulan Berkas Skripsi Unisbank Semarang";
    ...
    private function validation()
    {
        ...
        $this->form_validation->set_rules('s_nisn', 'NIM', 'trim|required');
        $this->form_validation->set_rules('s_name', 'Nama Mhs', 'trim|required|min_length[3]|max_length[50]');
        ...
        $this->form_validation->set_rules('s_grade', 'Inisial Mhs', 'trim|required|max_length[3]');
        $this->form_validation->set_rules('s_mid', 'Program Studi', 'trim|required|max_length[4]');
    ...
    public function add()
    {
        ...
        if ($this->validation()) {
            ...
            if ($data['s_nisn'] === $this->input->post('s_nisn', TRUE)) {
                ...
                if (!empty($s_skhun)) {
                    $config['upload_path'] = './uploads/fclembar/';
                    $config['allowed_types'] = 'pdf';
                    $config['file_ext_tolower'] = TRUE;
                    $config['max_size'] = '2048';
                    $config['overwrite'] = TRUE;
                ...
                if (!empty($s_foto) && !$this->upload->do_upload('s_foto')) {
                    ...
                    $this->session->set_flashdata('alert', success('Data Mhs berhasil ditambahkan.'));
                    $data['title'] = "Data ".self::$title;
                    $data['content'] = "dashboard/student";
                    redirect('student');
                }
    ...
    public function import()
    {
        ...
        if (isset($_POST['import'])) {
            ...
            if (empty($file)) {
                $this->session->set_flashdata('alert', error('Form file data Mhs wajib diisi!'));
                $data['title'] = "Impor Data ".self::$title;
                $data['form_title'] = "Impor Data Mhs";
                $data['action'] = site_url(uri_string());
            ...
            if (strtolower(end($eks)) === 'csv') {
                ...
                fclose($handle);
                $this->session->set_flashdata('alert', success('Data Mhs berhasil diimport.'));
                $data['title'] = "Impor Data ".self::$title;
                $data['content'] = 'dashboard/student';
                redirect('student');

            } else {
                $this->session->set_flashdata('alert', error('Format file yang diperbolehkan hanya *.csv.'));
                $data['title'] = "Impor Data ".self::$title;
                $data['form_title'] = "Impor Data Mhs";
                $data['action'] = site_url(uri_string());
                $data['content'] = 'dashboard/student-import';
    ...
    public function view()
    {
        ...
        $data['title'] = $data['student']['s_name']." &minus; Pengumpulan Berkas Skripsi Unisbank";
    ...
    public function status($s_id)
    {
        ...
        $this->session->set_flashdata('alert', success('Status data Mhs berhasil diperbarui.'));
        $data['title'] = "Data ".self::$title;
        $data['content'] = "dashboard/student";
        $this->load->view('dashboard/index', $data);
        redirect(site_url('student'));
    ...
    public function edit()
    {
        ...
        if ($this->validation()) {
            ...
            if (!empty($s_foto) && !$this->upload->do_upload('s_foto')) {
                ...
                $this->session->set_flashdata('alert', success('Data Mhs berhasil diperbarui.'));
                $data['title'] = "Data ".self::$title;
                $data['content'] = "dashboard/student";
                redirect(site_url('student'));
            }
    ...
    public function delete($s_id)
    {
        ...
        $this->session->set_flashdata('alert', success('Data Mhs berhasil dihapus.'));
        $data['title'] = "Data ".self::$title;
        $data['content'] = "dashboard/student";
        $this->load->view('dashboard/index', $data);
        redirect(site_url('student'));
    ...
    {
        $data['title'] = "Data Mhs Terhapus &minus; Pengumpulan Berkas Skripsi";
    ...
    public function restore()
    {
        ...
        $this->session->set_flashdata('alert', success('Data Mhs berhasil direstore.'));
        $data['title'] = "Data ".self::$title;
        $data['content'] = "dashboard/student-deleted";
